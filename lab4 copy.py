...
mode = 1
modelmode = [1,1,1] # 0-MLP;  1-CNN;  2-LogisticRegression;    3-Naive Bayes;    4-SVC;   5-QDA;  -1 -Load Model; 6-Mobilenet; 7- CNN2
...
if mode == 1:
    ...
    modelarrayG = [keras.models.load_model('my_modelo0.h5'), keras.models.load_model('bom1-82.h5'), keras.models.load_model('bom2-94.h5')]
    y_test_pred1 = modelarrayG[1].predict(x_test1.reshape(-1, 28, 28, 3))
    y_test_pred2 = modelarrayG[2].predict(x_test2.reshape(-1, 28, 28, 3))
    y_test_pred1 = [np.argmax(pred) for pred in y_test_pred1]
    y_test_pred2 = [np.argmax(pred) for pred in y_test_pred2]
    y_test_pred2 = [cat+3 for cat in y_test_pred2]
